...
export function ControlPalette({ definitions, onAdd }: Props) {
  ...
  return <aside className="left-panel"><div className="panel-heading"><div><span className="eyebrow">LIBRARY</span><h1>컨트롤</h1></div><span className="count">{definitions.length}</span></div>{renderCategory('basic', '기본')}{renderCategory('layout', '레이아웃')}</aside>
}
